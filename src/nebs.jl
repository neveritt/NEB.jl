...
#  @assert eltype(R) == T throw(ArgumentError())
  ...
  opt = optimize(df, Θ, Newton(), Optim.Options(g_tol = 1e-14))
  ...
  for i in 1:nᵤ
    ...
    uₜ = view(z,idx)
    ûₜ = view(û,idx)
    σᵥ[i] = (sum(abs2,uₜ-ûₜ) + trace(Pₜ))/N
  end

  # update output noise parameters
  Pₜ = view(P̂, nᵤ*N+(1:N), nᵤ*N+(1:N))
  σᵥ[end] = (sum(abs2,y-ŷ) + trace(Pₜ))/N
...
function _initial_NEBS{T}(y::AbstractMatrix{T}, u::AbstractMatrix{T},
  ...
  options = IdOptions(iterations = 20, estimate_initial=false)
